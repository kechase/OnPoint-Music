...
class HeadphonesCheck {
    ...
    _trialHandler(remaining, current = 0) {
      ...
      if (remaining <= 0) {
        ...
        // 🌟 Make the results globally accessible
        window.isHeadphones = this.isHeadphones;
        window.correctAnswers = this._currentRecord.numberCorrect;
        window.dispatchEvent(new Event("headphoneCheckComplete"));

        this.attemptRecord[this.attemptCount] = this._currentRecord;
        $('button:contains("Continue")')
            .button('option', 'disabled', false)
            .css('visibility', 'visible')
            //.trigger('click'); // this is creating a loop that says you haven't passed the headphones check, so disabled it
        return;
      }
            ...
            $('button[data-headphones-audio-control]')
                ...
                .on('click', function() {
                  ...
                  if (audioElements.length > 0) {
                    ...
                    console.warn('No audio elements found for headphones audio group');
                  }
                });
          }
        });
            ...
            if (--audioCount <= 0) {
              $('button[data-headphones-audio-control]')
                  ...
                  .on('click', function() {
                    ...
                    if (audioElements.length > 0) {
                    audioElements.each(function() {
                        const audio = this; // 'this' refers to the DOM element
                        audio.volume = audio.dataset.headphonesVolume;
